...
def equip_icon_url(equip_id: str) -> str:
    return f"{BASE_URL}/images/equipicon/{equip_id}.png"

def item_icon_url(item_id: str) -> str:
    return f"{BASE_URL}/images/itemicon/item_{item_id}.png"
...
def _parse_ability(soup: BeautifulSoup) -> WeaponAbility:
    _ID_RE = re.compile(r"/(\d+)\.html")
    # _pairs(dl)

    ability_block = soup.select_one(WEAPON_ABILITY_SELECTOR)
    metadata_block = ability_block.find_next("dd")

    id = _ID_RE.search(metadata_block.find_next("a").get("href")).group(1)
    name = metadata_block.select_one("p").get_text(strip=True)
    effects = []
...
def _parse_reforge(soup: BeautifulSoup) -> Reforge:
    reforge: Reforge = {}
    for dl in soup.select("dl.detail__evo"):
        ...
        return reforge
    return Reforge(before=None, after=None)
...
def parse_item(soup: BeautifulSoup, item_id: str):
    return
